...
async function seedViandas() {
  ...
  try {
    ...
    if (weeklyMenu) {
      console.log(`[seed-viandas] ✅ Weekly menu created for ${monday.toISOString().split('T')[0]} to ${sunday.toISOString().split('T')[0]}`)
    ...
    console.error('[seed-viandas] ❌ Viandas seed failed:', error)
    throw error
  } finally {
    await client.end()
  }
